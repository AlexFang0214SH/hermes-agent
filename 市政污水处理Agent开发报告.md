# 市政污水处理 Agent 开发计划
**基于 Hermes Agent 框架的市政污水处理Agent**  
（*Hermes Agent Framework for Municipal Wastewater Treatment*）
---
## AI Agent发展路径

## Agent发展阶段  
| 阶段 | 时间 | 核心特征 | 技术范式 | 代表框架 |
|------|------|---------|---------|---------|
| 被动式工具人 | 2023年 | 单步问答、固定Prompt | ReAct | LangChain早期版本 |
| 流程化执行者 | 2024年 | 工作流编排、多步任务 | Workflow | AutoGen、LangGraph |
| 自主规划者 | 2025年 | 自主决策、工具调用 | Autonomous | OpenClaw、Cloud Code |
| 自进化数字员工 | 2026年 | **持续学习、技能沉淀** | Self-Evolving | **Hermes Agent**、MAF |
...
|------|------|---------|
| 业务匹配度 | 30% | 是否满足核心业务需求 |
| 技术成熟度 | 25% | 框架稳定性、社区活跃度 |
| 集成能力 | 20% | MCP支持、系统对接能力 |
| 安全合规 | 15% | 企业级安全、审计能力 |
| 成本效益 | 10% | 开发成本、运维成本 |
...
| 企业类型 | 推荐框架 | 理由 |
|---------|---------|------|
| 个人开发者/初创公司 | Hermes Agent | 轻量快速、MCP原生、学习成本低 |
| 中小企业 | LangChain + MCP | 生态完善、灵活性高、社区支持好 |
| 大型企业/金融机构 | Microsoft Agent Framework | 企业级安全、长期支持、合规保障 |
| 研究机构/学术项目 | LangChain | 文档完善、研究案例多、实验灵活 |

> **Hermes Agent 框架选型：** 轻量快速、MCP原生、学习成本低，适合个人开发者/初创公司快速开发市政污水处理AI Agent
...
ProcSim 仿真计算后端是独立的 **FastAPI 微服务**，核心仿真引擎为 ProcSim（本地 whl 包），通过 Celery 实现异步并行计算，Tortoise ORM + MySQL 持久化仿真任务和结果，Redis 作为 Celery Broker 和缓存层。

**API 服务地址：根据实际部署配置**，默认端口 50001  
**API 版本：** `Process Simulation v0.1.0`，共 **340 个端点**，全部挂载在 `/api/v1/` 前缀下。

| 组件 | 版本 | 用途 |
|---|---|---|
| Python | 3.7+ | 开发语言 |
| FastAPI | 0.61.1 | Web 框架（REST API） |
| Tortoise ORM | 0.16.15 | 异步 ORM |
| aiomysql | 0.0.20 | MySQL 异步驱动 |
| Redis | 3.5.3 | Celery Broker + 缓存 |
| Celery | — | 异步任务队列（并行计算） |
| ProcSim | 本地 whl | 仿真计算引擎 |

### 7.2 六条集成路径

| 路径 | 机制 | ProcSim 接口 | 开发量 | 推荐度 |
|---|---|---|---|---|
| **P1. MCP Server** | 封装为 MCP 服务器 | REST API (FastAPI) | 中 | ★★★★★ |
| **P2. Hermes Plugin** | `ctx.register_tool()` | REST API (HTTP) | 中 | ★★★★ |
| **P3. Skill + execute_code** | Skill 指导写代码调用 | REST API (HTTP) | 低 | ★★★ |
...
### 7.4 MCP 工具设计（基于实际 API 契约对齐）

> 基于 ProcSim OpenAPI 文档（340 个端点）提取。
> ProcSim 的核心概念是 **Schema（仿真方案）**，而非简单的“提交任务”。仿真生命周期：
> **创建方案 → 配置参数 → 启动仿真 → 监控任务 → 获取结果**

**MCP 工具 → ProcSim API 端点映射表：**

| MCP 工具名 | 功能 | 对应 ProcSim API | 请求体 | 关键参数 |
|---|---|---|---|---|
| `procsim_start_simulate` | 启动离线仿真 | `POST /api/v1/simulate/start_simulate` | `SimulateCreate` | `schema_id` (str, 必填) |
| `procsim_start_online_simulate` | 启动在线仿真 | `POST /api/v1/simulate/start_simulate_online` | `SchemaUpdateMultiParamsValue` | `data` (array, 必填) |
| `procsim_batch_simulate` | 批量仿真 | `POST /api/v1/simulate/start_batch_simulate` | `BatchSimulateCreate` | `project_id` (int), `schema_ids` (array) |
| `procsim_result` | 查询仿真结果 | `GET /api/v1/simulate/result` | — | `schema_id` (query) |
| `procsim_all_result` | 查询方案全部结果 | `GET /api/v1/schemas/{schema_id}/all_result` | — | `schema_id` (path) |
| `procsim_active_tasks` | 查看运行中的任务 | `GET /api/v1/simulate/active_tasks` | — | — |
| `procsim_control` | 控制仿真（停止/暂停/恢复） | `POST /api/v1/simulate/control` | `Body_task_control` | `schema_id` (str), `command` (STOP\|PAUSE\|RESUME) |
| `procsim_terminate` | 终止任务 | `POST /api/v1/simulate/terminate_tasks` | `Body_terminate_tasks` | `schema_ids` (array) |
| `procsim_list_schemas` | 列出所有仿真方案 | `GET /api/v1/schemas/` | — | page/limit/keyword |
| `procsim_get_schema` | 获取方案详情 | `GET /api/v1/schemas/{schema_id}` | — | `schema_id` (path) |
| `procsim_get_params` | 获取方案参数 | `GET /api/v1/schemas/{schema_id}/params` | — | `schema_id`, model_id, type_id |
| `procsim_update_params` | 更新方案参数 | `PUT /api/v1/schemas/{schema_id}/params/{param_id}` | `SchemaParameterUpdate` | `schema_id`, `param_id` |
| `procsim_multi_update_params` | 批量更新参数 | `PUT /api/v1/schemas/{schema_id}/multi_param` | `SchemaUpdateMultiParameter` | `schema_id` |
| `procsim_chemicals` | 查询药剂信息 | `GET /api/v1/schemas/{schema_id}/chemicals` | — | `schema_id` |
| `procsim_energy_summary` | 查询能耗汇总 | `GET /api/v1/schemas/{schema_id}/energy_summary` | — | `schema_id` |
| `procsim_compose_add` | 添加对比分析 | `POST /api/v1/compose/{project_id}/add` | `ComposeCreate` | `project_id` |
| `procsim_compose_result` | 查询对比结果 | `GET /api/v1/compose/{compose_id}/result` | — | `compose_id`, `schema_id` |

**关键数据模型（已确认）：**

| 模型 | 字段 | 说明 |
|---|---|---|
| `SimulateCreate` | `schema_id` (str, 2-128字符, 必填) | 启动仿真的请求体 |
| `SchemaCreate` | `name` (str, 必填), `interval` (number, 必填), `group_id` (str, 必填), `terminal` (number, 默认1.0), `time_code` ("hours"\|"days", 默认"days"), `desc` (str), `template_id` (str) | 创建仿真方案 |
| `CommandEnum` | `STOP` / `PAUSE` / `RESUME` | 仿真控制命令 |
| `BatchSimulateCreate` | `project_id` (int, 必填), `schema_ids` (array) | 批量仿真请求 |
| `TaskStatusEnum` | 0=UNSTARTED, 1=PENDING, 2=PROGRESS, 3=SUCCESS, 4=FAILURE, 5=STOP/REVOKED, 6=PAUSE | 任务状态枚举（已确认） |
| `SchemaID` | Snowflake 64位算法（41位时间戳+5位数据中心ID+5位机器ID+12位序列号），返回字符串 | 方案 ID 生成规则 |

**ProcSim API 端点域分布（340 个端点）：**

| API 域 | 路径前缀 | 端点数（约） | MVP 需要 | 说明 |
|---|---|---|---|---|
| 仿真控制 | `/api/v1/simulate/*` | 17 | 是 | 启动/停止/查询仿真任务 |
| 方案管理 | `/api/v1/schemas/*` | 25+ | 是 | 仿真方案 CRUD + 参数管理 |
| 方案分组 | `/api/v1/schema_groups/*` | 10 | 是 | 方案分组管理 |
| 对比分析 | `/api/v1/compose/*` | 6 | 可选 | 多方案对比 |
| 碳足迹 | `/api/v1/carbon_footprint/*` | 15 | 否 | 碳排放核算 |
| 设备管理 | `/api/v1/equipment/*` | 15+ | 否 | 设备台账 |
| 大屏展示 | `/api/v1/screen/*` | 20 | 否 | Dashboard 数据 |
| 统计分析 | `/api/v1/statistic/*` | 20+ | 否 | 统计指标 |
| 沿途查看 | `/api/v1/along_way/*` | 5 | 否 | 工艺流程可视化 |
| 跨项目 | `/api/v1/between_project/*` | 4 | 否 | 跨项目对比 |
| 仿真指标 | `/api/v1/simulate_target/*` | 5 | 可选 | 仿真目标管理 |
| 用户管理 | `/api/v1/users/*` | 12 | 否 | 用户 CRUD |
| 角色管理 | `/api/v1/roles/*` | 10 | 否 | 角色 CRUD |
| 权限分享 | `/api/v1/share/*` | 5 | 否 | 共享/权限 |
| 文档管理 | `/api/v1/doc/*` | 6 | 否 | 文档 CRUD |
| 系统配置 | `/api/v1/system/*` + `/api/v1/sys_config/*` | 10 | 否 | 系统级设置 |
...
### 7.6 关键发现：ProcSim 是完整 SaaS 平台，不仅是仿真引擎

> **重要修正：** OpenAPI 契约显示 ProcSim 拥有 340 个端点，远超"纯仿真引擎"定位。它是一个涵盖方案管理、参数配置、仿真控制、碳足迹、设备管理、统计分析、用户权限的完整水务 SaaS 平台。

**对 MCP Server 设计的影响：**

- **MCP 工具数量**：MVP 只需封装 ~10 个核心端点（仿真控制 + 方案管理），而非 340 个全部暴露
- **核心概念修正**：ProcSim 的基本操作单元是 **Schema（仿真方案）**，不是简单的"提交任务"
- **仿真生命周期**：必须先有 Schema（方案）→ 配置参数 → 才能启动仿真
- **热启动路径**：通过 `start_simulate_online` 实现（`SchemaUpdateMultiParamsValue` 传入新参数值后启动）

**API 契约已确认事项（C-1 ~ C-6）：**

| 编号 | 问题 | 已确认结果 | 对 MCP Server 的影响 |
|---|---|---|---|
| C-1 | TaskStatusEnum 含义 | 0=UNSTARTED, 1=PENDING, 2=PROGRESS/STARTED, 3=SUCCESS, 4=FAILURE, 5=STOP/REVOKED, 6=PAUSE | MCP 层可直接映射状态名称返回给 Agent |
| C-2 | start_simulate vs start_simulate_simupy | 现阶段忽略区别 | MCP 统一使用 `procsim_start_simulate`，内部路由待后续优化 |
| C-3 | time_code 枚举值 | `"hours"` / `"days"` | Schema 参数校验时限定这两个值 |
| C-4 | API 认证方式 | 用户账号权限 | MCP Server 需代理用户登录获取 Token，或配置服务账号 |
| C-5 | API 速率限制 | 暂不考虑，通过多弹性计算节点解决并行 | MCP 层无需实现限流器，但应预留限流开关 |
| C-6 | schema_id 生成规则 | Snowflake 雪花算法（64位：1符号+41时间戳+5数据中心+5机器+12序列号），返回字符串 | MCP 工具参数类型为 string；ProcSim 侧生成，MCP 不自行构造 |
...
| **ProcSim 能否集成** | 可行，ProcSim 是 340 端点的完整 SaaS 平台（FastAPI），MVP 封装 ~10 个核心端点，以 Schema（仿真方案）为核心概念 |
...
| 确认项 | 内容 |
|---|---|
| 部署方式 | 独立 FastAPI 微服务，REST API 接口 |
| 后端框架 | FastAPI 0.61.1（Python 3.7+） |
| ORM | Tortoise ORM 0.16.15 + aiomysql 0.0.20 |
| 数据库 | MySQL（任务状态 + 仿真结果持久化） |
| 任务队列 | Celery + Redis 3.5.3（异步并行计算） |
| 仿真引擎 | ProcSim（本地 whl 包） |
| 许可证模式 | 支持私有化与云端部署，多计算节点弹性部署 |
| 单次仿真耗时 | 秒级 ~ 分钟级 |
| 仿真结果数据量 | ≤ 500 字段，每字段为时间序列数据（与仿真时长相关） |
| 热启动 | 支持基于上次结果继续仿真 |
| 工艺模型 | ASM1 / ASM2d / ASM3（ProcSim 内置），支持模型类型扩展 |
...
- FastAPI REST API → **MCP Server 通过 HTTP 调用 ProcSim FastAPI 端点**，而非 Python SDK 直接嵌入
- Celery + Redis → ProcSim 自身通过 Celery Worker 处理并发，Redis 作为 Broker；Hermes MCP 端只需**提交任务 + 轮询结果**
- MySQL + Tortoise ORM → ProcSim 自带数据持久化，任务状态和结果存 MySQL，**Hermes 无需额外数据库**
- ProcSim（本地 whl） → 仿真引擎以本地包形式部署在 ProcSim 容器内，**无需在 Hermes 侧安装**
...
**ProcSim FastAPI 服务内部架构（已确认）：**

```
┌─ ProcSim FastAPI Service ──────────────────────────────┐
│                                                         │
│  FastAPI (0.61.1)                                       │
│  ├── REST API 端点 (submit/result/cancel/models)        │
│  ├── Tortoise ORM → MySQL (任务+结果持久化)              │
...
└─────────────────────────────────────────────────────────┘
         ↑ HTTP                    ↑ HTTP
         │                         │
   Hermes MCP Server           Dify 工作流
```

**Hermes 侧集成架构（MCP Server 作为轻量 HTTP 客户端）：**

- Hermes MCP Server 仅需 `httpx` 或 `aiohttp` 调用 FastAPI 端点
- 无需在 Hermes 侧安装 ProcSim whl（仿真引擎完全隔离在 ProcSim 容器内）
- 无需直连 MySQL（所有数据访问通过 FastAPI 端点）

**MCP 工具设计（基于实际 API 契约对齐，详见 §7.4）：**

| MCP 工具 | 对应 ProcSim API | 核心参数 | 返回策略 |
|---|---|---|---|
| `procsim_start_simulate` | `POST /simulate/start_simulate` | `schema_id` | 秒级仿真直接返回结果；分钟级轮询 active_tasks → result |
| `procsim_start_online_simulate` | `POST /simulate/start_simulate_online` | `data` (array) | 同上（热启动路径） |
| `procsim_result` | `GET /simulate/result` | `schema_id` | 返回摘要（关键出水指标 + 合规判断） |
| `procsim_active_tasks` | `GET /simulate/active_tasks` | — | 返回任务状态列表 |
| `procsim_control` | `POST /simulate/control` | `schema_id`, `command` (STOP/PAUSE/RESUME) | 返回控制结果 |
| `procsim_terminate` | `POST /simulate/terminate_tasks` | `schema_ids` (array) | 释放 Celery 资源 |
| `procsim_list_schemas` | `GET /schemas/` | page/limit/keyword | 返回方案列表 |
| `procsim_get_params` | `GET /schemas/{id}/params` | `schema_id` | 返回参数列表 |
| `procsim_update_params` | `PUT /schemas/{id}/params/{pid}` | `schema_id`, `param_id` | 参数更新确认 |
| `procsim_compose_result` | `GET /compose/{id}/result` | `compose_id`, `schema_id` | 对比分析结果 |

**API 服务地址：** 根据具体配置获取
**OpenAPI 规范：** `/openapi.json`（340 个端点，完整契约详见 §7.4）
...
services:
  # ---- Hermes Agent 层 ----
  hermes-gateway:      # Hermes Agent 主服务（Python 3.11+）
  
  # ---- ProcSim 仿真计算层 ----
  procsim-api:         # ProcSim FastAPI 服务 (0.61.1)
  procsim-worker:      # ProcSim Celery Worker（执行仿真计算）
  procsim-mysql:       # ProcSim 专用 MySQL（Tortoise ORM + aiomysql）
  ...
  # ---- 共享基础设施层 ----
  redis:               # Celery Broker (procsim) + Dify 缓存
  postgres:            # Dify 数据库
  nginx:               # 反向代理 + 认证
```
...
| Day | 任务 | 交付物 |
|---|---|---|
| 9 | API 契约对齐：与 ProcSim 团队确认 C-1~C-6 问题，确认认证方式和 TaskStatusEnum 含义 | 契约对齐文档 |
| 10-11 | ProcSim MCP Server 开发（基于实际 API 契约：start_simulate/result/active_tasks/control + schemas CRUD） | MCP 服务器 |
| 12-13 | 异步调用模式（active_tasks 轮询 → result 获取 → 摘要返回） | 异步调用链 |
| 14 | 结果摘要逻辑（500字段时间序列 → 关键出水指标 + 合规判断） | 摘要模块 |
...
| 风险 | 说明 | 缓解 |
|---|---|---|
| **Dify 直接调 ProcSim 的链路复杂度** | Dify→ProcSim 和 Hermes→ProcSim 两条链路，需统一任务 ID 和状态 | ProcSim FastAPI 侧实现统一的任务管理 API |
...
| **Python 版本兼容性** | ProcSim 后端 Python 3.7+，Hermes 要求 Python 3.11+ | 容器化隔离，各自使用独立 Python 环境，通过 HTTP 通信 |
| **ProcSim 本地 whl 管理** | 本地 whl 包不在 PyPI 上，需手动管理版本和部署 | ProcSim 容器镜像内置 whl；版本管理通过 ProcSim 侧 Git 控制 |
| **MySQL 额外运维负担** | ProcSim 依赖 MySQL（Tortoise ORM + aiomysql），增加数据库运维 | 使用 Docker Compose 托管 MySQL 容器；备份策略随 ProcSim 团队对齐 |
| **ProcSim API 契约对齐** | OpenAPI 有 340 个端点，部分枚举含义已确认（C-1~C-6），`start_simulate` vs `start_simulate_simupy` 区别现阶段忽略 | 契约对齐已完成，MVP 封装 10 个核心端点；认证方式需代理服务账号 |
| **ProcSim 是 SaaS 而非纯引擎** | 原计划将 ProcSim 视为纯计算服务，实际它自带用户/角色/权限/方案管理，与 Hermes 能力重叠 | 明确边界：ProcSim 负责仿真计算和数据持久化，Hermes 负责 AI 推理和用户交互层 |
